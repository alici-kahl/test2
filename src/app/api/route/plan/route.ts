...
async function callValhalla(
  ...
  try {
    ...
    // NIEMALS ungefangen werfen: immer JSON-kompatibel zurückgeben
    try {
    ...
    return { geojson: { type: "FeatureCollection", features: [] }, error: String(e), meta: { ok: false } };
  } finally {
    clearTimeout(timer);
  }
}
...
function computeRouteStats(
  ...
  try {
    ...
    // Wenn buffer() hier scheitert, wollen wir nicht crashen → dann einfach keine Warnungen
...
/**
 * Fix: Statt "die ersten N Boxen" nehmen wir N Boxen gleichmäßig verteilt über die Route,
...
function pickSpreadBoxes<T>(arr: T[], max: number): T[] {
  ...
  // echtes Dedupe: nach Inhalt, nicht nach Index
  const seen = new Set<string>();
  return out.filter((x) => {
...
export async function POST(req: NextRequest) {
  // WICHTIG: alles kapseln, damit die Route niemals HTML/Text durch ungefangene Fehler liefert
  try {
    ...
    // Harte Deadline deutlich unter maxDuration, um 504 zu vermeiden
    const HARD_DEADLINE_MS = 52_000;
    const t0 = Date.now();
    const timeLeft = () => HARD_DEADLINE_MS - (Date.now() - t0);

    // Wenn der Client breitere Fahrzeuge testet, wird es iterativ teurer → etwas konservativer timen
    const VALHALLA_TIMEOUT_MS = Math.min(12_000, Math.max(8_000, body.valhalla_soft_max ? 12_000 : 12_000));
    ...
    // Avoid-Puffer: Basis aus Roadworks-Buffer + zusätzliche Sicherheitsmarge für breite Fahrzeuge
    ...
    // Helper: wenn wenig Zeit übrig ist, brechen wir kontrolliert ab und liefern bestmögliches Ergebnis
    const shouldStopSoon = () => timeLeft() < VALHALLA_TIMEOUT_MS + 2_500;
    ...
    if (approxKm >= LONG_ROUTE_KM) {
      if (shouldStopSoon()) {
        return NextResponse.json({
          meta: {
            ...
            status: "BLOCKED",
            clean: false,
            error: "Zeitbudget zu klein für Berechnung.",
            iterations: totalIterations,
            avoids_applied: 0,
            bbox_km_used: null,
            fallback_used: true,
            phases: [{ phase: "FAST_PATH", result: "ABORT_EARLY" }],
          },
      ...
      const res0 = await callValhalla(origin, plannerReqBase, [], Math.min(VALHALLA_TIMEOUT_MS, Math.max(8_000, timeLeft() - 2_500)), false);
      ...
      // Budget-sicher: max 4 Boxen, über die Route verteilt
      ...
      // CPU-Schutz: wenn Roadworks riesig sind, cap bevor booleanIntersects explodiert
      const mergedCapped = merged.slice(0, 2200);

      const obstacles: Feature<any>[] = prioritizeObstacles(mergedCapped, start, end, corridorKm, 1400);
      ...
      const MAX_ITER_FAST = 6; // etwas konservativer gegen 504
      ...
      for (let i = 0; i < MAX_ITER_FAST; i++) {
        ...
        // Option A: escape_mode true + Alternativen
    ...
    for (const bboxKm of BBOX_STEPS_KM) {
      if (shouldStopSoon()) break;

      let bbox: [number, number, number, number];
      try {
        bbox = makeSafeBBox(start, end, bboxKm);
      } catch {
        // wenn Turf buffer/bbox knallt, skip diesen Schritt statt crashen
        phases.push({ phase: "STRICT", bbox_km: bboxKm, result: "SKIP_BBOX_ERROR" });
        continue;
      }
      ...
      // CPU-Schutz: cap Roh-Obstacles
      const rawObstacles: Feature<any>[] = ((rw?.features ?? []) as Feature<any>[]).slice(0, 5000);
    ...
    if (!best?.route?.features?.length && !shouldStopSoon()) {
    ...
    // WICHTIG: niemals HTML/Plaintext an den Client → immer JSON
    return NextResponse.json(
      {
        meta: {
          source: "route/plan-v21-least-roadworks",
          status: "BLOCKED",
          clean: false,
          error: `Serverfehler in /api/route/plan: ${String(e)}`,
        },
        geojson: { type: "FeatureCollection", features: [] },
        blocking_warnings: [],
        geojson_alts: [],
